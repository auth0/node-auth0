...
api.sendVerificationEmail = function(accessToken, userId, done) {
  authRequest(accessToken).post({
    url: this.apiUrl + '/users/' + userId + '/send_verification_email'
  }, function (err, res, body) {
    if (err) { return done(err); }
    if (res.statusCode === 404) { return done(); }
    if (res.statusCode.toString().substr(0, 1) !== '2') { return done(new ApiError(body.detail || body, res.statusCode)); }
    done(null, body);
  });
};
