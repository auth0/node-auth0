...
export class EventStreamsClient {
    ...
    private async __getStats(
        ...
        const _metadata: core.EndpointMetadata = {
            security: [{ bearerAuth: [] }, { oAuth2ClientCredentials: ["read:event_deliveries"] }],
        };
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.ManagementEnvironment.Default,
                `event-streams/${core.url.encodePathParam(id)}/stats`,
            ),
            method: "GET",
            headers: _headers,
            queryParameters: { ..._queryParams, ...requestOptions?.queryParams },
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/event-streams/{id}/stats");
    }
