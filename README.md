...
- [Authentication Migration Guide](https://github.com/auth0/node-auth0/blob/master/AUTH_MIGRATION_GUIDE.md) - migrate the Authentication API layer to `@auth0/auth0-auth-js` / `@auth0/auth0-server-js`. Start here for the P0 OIDC migration; the [`migration/`](https://github.com/auth0/node-auth0/tree/master/migration) directory holds the incremental flow and session pages. Migrating with an AI agent? Point it at the [`auth0` migration skill](https://github.com/auth0/agent-skills) (intent `migrate-node-auth0`).
