...
describe("id-token-validator", () => {
    beforeEach(async () => {
        // Set up the JWKS endpoint mock for all tests
        const jwk = await jose.exportJWK(await jose.importSPKI(TEST_PUBLIC_KEY, "RS256"));
        nock(URL)
            .persist()
    ...
    it("rejects when azp is not present when more audiences are provided", async () => {
        ...
        await expect(idTokenValidator.validate(jwt)).rejects.toThrowError(/\(azp\) claim must be a string/);
    });
    ...
    it("validates nonce when in token", async () => {
        ...
        const jwt = await sign({ payload: { nonce: "foo" } });
        await expect(idTokenValidator.validate(jwt)).rejects.toThrowError(/\(nonce\) claim mismatch in the ID token/);
    });
