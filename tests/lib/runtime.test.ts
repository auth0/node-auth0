...
import {
    ...
    UserInfoError,
    AuthApiError,
    ResponseError as NewResponseError,
} from "../../src/index.js";
import { ManagementClient, ManagementApiError, CustomDomainHeader, ResponseError } from "auth0-legacy";
...
describe("Runtime", () => {
    ...
    it("should apply array of query params with multiple params", () => {
        const params = applyQueryParams({ foo: ["bar", "baz"] }, [{ key: "foo", config: { isArray: true } }]);
...
describe("Runtime for ManagementClient", () => {
    ...
    it("should throw an ManagementError when backend provides known error details", async () => {
        nock(URL, { encodedQueryParams: true }).get("/clients").reply(428, {
            error: "test error",
    ...
    it("should throw an ManagementError and fallback to the response status code when statusCode omitted from response", async () => {
        nock(URL, { encodedQueryParams: true }).get("/clients").reply(428, {
            error: "test error",
            errorCode: "test error code",
            message: "test message",
    ...
    it("should add the telemetry by default", async () => {
        const request = nock(URL)
            .get("/clients")
            .matchHeader("Auth0-Client", (val) => !!val) // Just check that Auth0-Client header exists
            .reply(200, []);
    ...
    it("should add the telemetry in workerd contexts", async () => {
        // Mock the RUNTIME module to simulate workerd environment
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const originalRUNTIME = (utils as any).RUNTIME;

        // Mock the utils module's generateClientInfo function
        const mockGenerateClientInfo = jest.spyOn(utils, "generateClientInfo");
        ...
        try {
            ...
            // Restore the mock
            mockGenerateClientInfo.mockRestore();
        }
    });
...
describe("Runtime for AuthenticationClient", () => {
    ...
    it("should not retry if not enabled", async () => {
        ...
        try {
            ...
            if (e instanceof NewResponseError) {
                expect(e.statusCode).toBe(429);
                expect(request.isDone()).toBe(false);
            } else {
                expect(e).toBeInstanceOf(NewResponseError);
            }
        }
    });
