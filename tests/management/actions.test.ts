...
describe("ActionsManager", () => {
        ...
        describe("#getAll", () => {
            const data = [
                {
                    ...
                    supported_triggers: [{ id: "post-login", version: "v1", status: "pending", runtimes: ["node12"] }],
                    runtime: "node12",
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                ...
                actions.getAll().then((credentials) => {
                    expect(Array.isArray(credentials.data.actions)).toBe(true);
                    ...
                    expect(credentials.data.actions?.[0].dependencies?.[0].name).toBe(data[0].dependencies[0].name);
                    ...
                    expect(credentials.data.actions?.[0].secrets?.[0].updated_at).toBe(data[0].secrets[0].updated_at);
                    ...
                    expect(credentials.data.actions?.[0].all_changes_deployed).toBe(data[0].all_changes_deployed);
        ...
        describe("#get", () => {
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                ...
                actions.get({ id: data.id as string }).then((credentials) => {
                    ...
                    expect(credentials.data.supported_triggers?.[0].version).toBe(data.supported_triggers?.[0].version);
                    expect(credentials.data.supported_triggers?.[0].status).toBe(data.supported_triggers?.[0].status);
        ...
        describe("#create", () => {
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                actions.create(data).then((action) => {
                    ...
                    expect(action.data.supported_triggers?.[0].version).toBe(data.supported_triggers[0].version);
                    expect(action.data.supported_triggers?.[0].status).toBe(data.supported_triggers[0].status);
    ...
    describe("action versions", () => {
        describe("#getVersions", () => {
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                ...
                actions.getVersions({ actionId }).then((versions) => {
                    ...
                    expect(versions.data.versions?.[0].dependencies?.[0].name).toBe(data[0].dependencies?.[0].name);
        ...
        describe("#getVersion", () => {
            ...
            it("should return a promise if no callback is given", (done) => {
                actions.getVersion({ actionId, id }).then(done.bind(null, null)).catch(done.bind(null, null));
            });
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                ...
                actions.getVersion({ actionId, id }).then((version) => {
                    ...
                    expect(version.data.supported_triggers?.[0].version).toBe(data.supported_triggers?.[0].version);
                    expect(version.data.supported_triggers?.[0].status).toBe(data.supported_triggers?.[0].status);
        ...
        describe("#deployVersion", () => {
            ...
            it("should return a promise when no callback is given", (done) => {
                actions.deployVersion({ actionId: action_id, id: version_id }, {}).then(done.bind(null, null));
            });

            it("should perform a post request", (done) => {
                actions.deployVersion({ actionId: action_id, id: version_id }, { update_draft: true }).then(() => {
                    expect(request.isDone()).toBe(true);
            ...
            it("should pass any errors to the promise catch handler", (done) => {
                ...
                nock(API_URL).post(`/actions/actions/${action_id}/versions/${version_id}/deploy`).reply(500, {});
    ...
    describe("triggers", () => {
        ...
        describe("#getAllTriggers", () => {
            ...
            it('should pass the body of the response to the "then" handler', (done) => {
                ...
                actions.getAllTriggers().then((triggers) => {
                    expect(Array.isArray(triggers.data.triggers)).toBe(true);
        ...
        describe("#getTriggerBindings", () => {
            ...
            beforeEach(() => {
                request = nock(API_URL).get(`/actions/triggers/${trigger_id}/bindings`).reply(200, { bindings: data });
            });
