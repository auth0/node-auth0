...
describe("PromptsManager", () => {
    ...
    describe("#getCustomTextByLanguage", () => {
        ...
        it("should return a promise if no callback is given", (done) => {
            prompts.getCustomTextByLanguage(params).then(done.bind(null, null)).catch(done.bind(null, null));
        });
    ...
    describe("#updateCustomTextByLanguage", () => {
        ...
        it("should validate empty prompt parameter", async () => {
            await expect(prompts.updateCustomTextByLanguage({} as any, {})).rejects.toThrowError(RequiredError);
        });

        it("should validate empty language parameter", async () => {
            await expect(prompts.updateCustomTextByLanguage({ prompt: "common" } as any, {})).rejects.toThrowError(
                RequiredError,
            );
        });
        ...
        it("should send the payload to the body", (done) => {
            ...
            const request = nock(API_URL)
                .put("/prompts/consent/custom-text/en", (body) => body && body.login && body.login.title === "Hello!")
                .reply(200, {});
    ...
    describe("#getAllRenderingSettings", () => {
        ...
        it('should pass the body of the response to the "then" handler', (done) => {
            ...
            prompts.getAllRenderingSettings().then((provider) => {
                expect(provider.data[0].default_head_tags_disabled).toEqual(data[0].default_head_tags_disabled);
