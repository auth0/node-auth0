...
import { RolesManager, ManagementClient, RequiredError } from "auth0-legacy";
...
describe("RolesManager", () => {
    ...
    describe("#getAll", () => {
        ...
        it("should include the token in the Authorization header", async () => {
            ...
            const request = nock(API_URL).get("/roles").matchHeader("Authorization", `Bearer ${token}`).reply(200, []);
    ...
    describe("#create", () => {
        ...
        it("should include the token in the Authorization header", (done) => {
            ...
            const request = nock(API_URL).post("/roles").matchHeader("Authorization", `Bearer ${token}`).reply(200, {});
