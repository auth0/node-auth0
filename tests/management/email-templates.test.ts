...
describe("EmailTemplatesManager", () => {
    ...
    describe("#update", () => {
        ...
        beforeEach(() => {
            request = nock(API_URL).patch(`/email-templates/${TEMPLATE_NAME}`, patchData).reply(200, response);
        });

        it("should return a promise if no callback is given", (done) => {
            emailTemplates.update(DEFAULT_PARAMS, patchData).then(done.bind(null, null)).catch(done.bind(null, null));
        });
        ...
        it("should include the new data in the body of the request", (done) => {
            ...
            const request = nock(API_URL).patch(`/email-templates/${TEMPLATE_NAME}`, patchData).reply(200, response);
