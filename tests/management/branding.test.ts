...
describe("BrandingManager", () => {
    ...
    describe("#getUniversalLoginTemplate", () => {
        ...
        beforeEach(() => {
            request = nock(API_URL).get("/branding/templates/universal-login").reply(200, { body: "test" });
        });
    ...
    describe("#deleteUniversalLoginTemplate", () => {
        ...
        it("should return a promise if no callback is given", (done) => {
            branding.deleteUniversalLoginTemplate().then(done.bind(null, null)).catch(done.bind(null, null));
        });
    ...
    describe("#getTheme", () => {
        ...
        it('should pass the body of the response to the "then" handler', async () => {
            const data = JSON.parse(
                (await util.promisify(fs.readFile)(path.join(testDir, "../data/theme.json"))) as unknown as string,
    ...
    describe("#configurePhoneProvider", () => {
        ...
        it("should return a promise if no callback is given", (done) => {
            branding.configurePhoneProvider(data).then(done.bind(null, null)).catch(done.bind(null, null));
        });
    ...
    describe("#updatePhoneProvider", () => {
        ...
        beforeEach(() => {
            request = nock(API_URL).patch(`/branding/phone/providers/${requestParameters.id}`).reply(200, response);
        });
    ...
    describe("#deletePhoneProvider", () => {
        ...
        it("should perform a DELETE request to /branding/phone/providers/:id", (done) => {
            ...
            const request = nock(API_URL).delete(`/branding/phone/providers/${requestParameters.id}`).reply(204);
    ...
    describe("#getPhoneProvider", () => {
        ...
        beforeEach(() => {
            nock.cleanAll();
            request = nock(API_URL).get(`/branding/phone/providers/${requestParameters.id}`).reply(200, response);
        });

        it("should return a promise if no callback is given", (done) => {
            branding.getPhoneProvider(requestParameters).then(done.bind(null, null)).catch(done.bind(null, null));
        });

        it("should pass any errors to the promise catch handler", (done) => {
            nock.cleanAll();
            request = nock(API_URL).get(`/branding/phone/providers/${requestParameters.id}`).reply(500, {});
