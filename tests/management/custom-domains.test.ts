...
describe("CustomDomainsManager", () => {
    ...
    describe("#getAll", () => {
        ...
        it('should pass the body of the response to the "then" handler', (done) => {
            customDomains.getAll().then((customDomains: ApiResponse<GetCustomDomains200Response>) => {
                expect(Array.isArray(customDomains.data)).toBe(true);
    ...
    describe("#get", () => {
        ...
        beforeEach(() => {
            request = nock(API_URL).get(`/custom-domains/${response.custom_domain_id}`).reply(200, response);
        });
    ...
    describe("#verify", () => {
        ...
        it("should return a promise if no callback is given", (done) => {
            customDomains.verify({ id: data.id }, {}).then(done.bind(null, null)).catch(done.bind(null, null));
        });
